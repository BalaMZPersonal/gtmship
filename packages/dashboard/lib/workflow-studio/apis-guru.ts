 ...
 */
const DIRECT_SPEC_URLS: Record<
  string,
  { primary: string; subApis?: Array<{ key: string; specUrl: string; title: string }> }
> = {
  hubspot: {
    primary: `${HUBSPOT_SPEC_BASE}/CRM/Objects/Rollouts/424/v3/objects.json`,
    subApis: [
      { key: "crm-objects", specUrl: `${HUBSPOT_SPEC_BASE}/CRM/Objects/Rollouts/424/v3/objects.json`, title: "CRM Objects (Contacts, Companies, Deals)" },
      { key: "crm-contacts", specUrl: `${HUBSPOT_SPEC_BASE}/CRM/Contacts/Rollouts/424/v3/contacts.json`, title: "CRM Contacts" },
      { key: "crm-companies", specUrl: `${HUBSPOT_SPEC_BASE}/CRM/Companies/Rollouts/424/v3/companies.json`, title: "CRM Companies" },
      { key: "crm-deals", specUrl: `${HUBSPOT_SPEC_BASE}/CRM/Deals/Rollouts/424/v3/deals.json`, title: "CRM Deals" },
      { key: "crm-tickets", specUrl: `${HUBSPOT_SPEC_BASE}/CRM/Tickets/Rollouts/424/v3/tickets.json`, title: "CRM Tickets" },
      { key: "crm-properties", specUrl: `${HUBSPOT_SPEC_BASE}/CRM/Properties/Rollouts/145899/v3/properties.json`, title: "CRM Properties" },
      { key: "crm-pipelines", specUrl: `${HUBSPOT_SPEC_BASE}/CRM/Pipelines/Rollouts/145896/v3/pipelines.json`, title: "CRM Pipelines" },
      { key: "crm-associations", specUrl: `${HUBSPOT_SPEC_BASE}/CRM/Associations/Rollouts/120891/v3/associations.json`, title: "CRM Associations" },
      { key: "crm-lists", specUrl: `${HUBSPOT_SPEC_BASE}/CRM/Lists/Rollouts/144891/v3/lists.json`, title: "CRM Lists" },
      { key: "crm-imports", specUrl: `${HUBSPOT_SPEC_BASE}/CRM/Imports/Rollouts/144903/v3/imports.json`, title: "CRM Imports" },
    ],
  },
};
 ...
 * When an array is provided, the first entry is the primary (most commonly needed) sub-API.
...
let _cachedDirectory: { data: ApisGuruDirectory; fetchedAt: number } | null =
  null;
...
async function fetchDirectory(): Promise<ApisGuruDirectory | null> {
  if (
    _cachedDirectory &&
    Date.now() - _cachedDirectory.fetchedAt < DIRECTORY_TTL_MS
  ) {
  ...
  _loading = (async () => {
    try {
      ...
      console.warn(
        "[apis-guru] Directory fetch error:",
 ...
 */
export async function resolveOpenApiSpecUrl(
  slug: string
): Promise<string | null> {
...
/**
 * List all available sub-APIs for a provider with multiple specs (e.g., HubSpot, Twilio).
 */
export async function listAvailableSubApis(
  slug: string
): Promise<Array<{ key: string; specUrl: string; title?: string }>> {
  // Check direct spec URLs first
  const direct = DIRECT_SPEC_URLS[slug];
  if (direct?.subApis) {
    return direct.subApis;
  }
  ...
  const results: Array<{ key: string; specUrl: string; title?: string }> = [];
  for (const key of keys) {
    const entry = directory[key];
    if (!entry) continue;
    const specUrl = extractSpecUrl(entry);
    if (!specUrl) continue;
    const preferred = entry.versions[entry.preferred];
    results.push({
      key,
      specUrl,
      title: preferred?.info?.title,
    });
  }
